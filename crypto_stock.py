...
class CryptoStockSim(CryptoStock):
    """
    Simulation version of CryptoStock that updates quantity without placing real orders.
    """

    def __init__(self, asset: str, qty: float = 0.0) -> None:
        super().__init__(asset, qty)

    def buy(self, amount: float) -> None:
        qty = amount / self.curr_price
        self.quantity += qty
        logger.info(f"**** BUY {self.name} on {self.curr_date}: {qty} acquired; total quantity now {self.quantity}")

    def sell(self, share: float) -> float:
        if not (0 < share <= 1):
            raise ValueError("Share must be between 0 and 1.")
        qty = self.quantity * share
        if qty == 0:
            return 0.0
        self.quantity -= qty
        logger.info(f"*** SELL {self.name} on {self.curr_date}: {qty} sold; total quantity now {self.quantity}")
        return self.curr_price * qty
